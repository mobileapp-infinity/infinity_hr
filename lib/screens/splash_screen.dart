...
class _SplashScreenState extends State<SplashScreen> {
  ...
  void apicall() async{
    AppDialog.showCustomizeProgressDialog(context: context, msg: 'Please wait...');
    Response response = await DioClient.getDioClient()!.get(ApiUrls.BASE_URL + 'Get_app_version');
    Navigator.of(context).pop();
    if(response.statusCode == 200){
      List<version> updateverlist =
      ...
      if(updateverlist.isNotEmpty){
        ...
        _showToast(message: 'Something went wrong,Please try again later ${response.statusCode}');
      }
    }else{
      _showToast(message: 'Something went wrong,Please try again later ${response.statusCode}');
    }
